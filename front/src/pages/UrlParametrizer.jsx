...
const UrlParametrizer = () => {
  ...
  const [agencyName] = useState('FCBHEALTH');
  ...
  const handleParametrize = async () => {
    ...
    selectedValues['agencia'] = agencyName;
    ...
    try {
      ...
      const { parametrizedUrl } = response.data;
      setResultUrl(parametrizedUrl);  // Armazene a URL completa
      message.success('URL parametrizada com sucesso!');
    } catch (error) {
      console.error('Erro ao gerar URL:', error);
      message.error('Erro ao gerar URL parametrizada.');
    }
  };
  ...
  const handleSaveUrl = async () => {
    ...
    try {
      await axios.post(`${API_URL}/save-url`, savedUrl);
      ...
      message.error('Erro ao salvar URL.');
      console.error(error);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`${API_URL}/urls/${selectedUrl.id}`); // <- ajustado aqui

      const updatedUrls = savedUrls.filter(url => url.id !== selectedUrl.id);
      setSavedUrls(updatedUrls);
      setIsModalVisible(false);
      message.success('URL excluída com sucesso!');
    } catch (error) {
      console.error('Erro ao excluir URL:', error);
      message.error('Erro ao excluir a URL');
    }
  };

  const handleCopyUrl = () => {
    if (resultUrl) {
      navigator.clipboard.writeText(resultUrl);  // Use resultUrl aqui
  ...
  const handleEditCampaignName = async (e) => {
    if (e.key === 'Enter' && selectedUrl?.id) {
      try {
        ...
        message.error('Erro ao atualizar nome da campanha.');
      }
    }
  };

  return (
    <Row gutter={[24, 24]} justify="center" style={{ padding: '20px', minHeight: '100vh', margin: 0 }}>
      <Col xs={24} md={6}>
                  ...
                  title={
                    isEditing && selectedUrl?.url === savedUrl.url ? (
                      <Input
                        ...
                        onKeyDown={(e) => handleEditCampaignName(e)}
                        autoFocus
      ...
      <Modal
        ...
        footer={[
          <Button key="delete" danger onClick={() => handleDelete(savedUrls.indexOf(selectedUrl))}>
            Excluir
          </Button>,
